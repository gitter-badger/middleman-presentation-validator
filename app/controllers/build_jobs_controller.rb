class BuildJobsController < ApplicationController
  before_action :set_build_job_job, only: [:show, :edit, :update, :destroy]
  ...
  def create
    @build_job = BuildJob.new(build_job_job_params)
  ...
  #def update
  #  respond_to do |format|
  #    if @build_job.update(build_job_job_params)
  ...
  private
    ...
    def set_build_job_job
      @build_job = BuildJob.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def build_job_job_params
      params.require(:build_job).permit(:date, :status_id, :output)
    end
end
